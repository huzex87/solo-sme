...
const METRICS = [
    ...
    { label: "Unique Customers", value: "0", sub: "No customers yet", icon: Users, color: "var(--accent-customers)", bg: "var(--accent-lt)" },
    ...
    return (
        <div className="animate-entrance" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                <div>
                    <h2 style={{ fontSize: 20, fontWeight: 800, color: "var(--ink)", letterSpacing: "-0.03em", margin: 0 }}>Analytics</h2>
                    ...
                    {PERIODS.map((p) => (
                        <button
                            ...
                            style={{
                                ...
                                boxShadow: period === p ? "0 1px 4px rgba(0,121,140,0.2)" : "none",
                            }}
                        >
                            {p}
                        </button>
                ...
                {METRICS.map((m) => {
                    ...
                    return (
                        <div
                            key={m.label}
                            className="card"
                            style={{ padding: 20, borderRadius: "var(--rl)", position: "relative", overflow: "hidden" }}
                        >
                            ...
                            <div style={{ fontSize: 12, color: "var(--muted)", fontWeight: 600, marginTop: 4 }}>{m.label}</div>
                            <div style={{ fontSize: 11, color: "var(--ghost)", fontWeight: 500, marginTop: 2 }}>{m.sub}</div>
                        </div>
                    );
                })}
            </div>
            ...
            <div className="card" style={{ padding: 24, borderRadius: "var(--rl)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                    <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--ink)", margin: 0 }}>Revenue Over Time</h3>
                    <span style={{ fontSize: 11, color: "var(--ghost)", fontWeight: 600 }}>{period}</span>
                </div>
                <div
                    style={{
                        height: 200,
                ...
                >
                    ...
                    <p style={{ fontSize: 11, color: "var(--ghost)", maxWidth: 240, textAlign: "center", lineHeight: 1.5 }}>
                        Revenue chart appears once your first sale is recorded.
                    </p>
                </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div className="card" style={{ padding: 20, borderRadius: "var(--rl)" }}>
                    <h3 style={{ fontSize: 13, fontWeight: 700, color: "var(--ink)", margin: "0 0 14px" }}>Top Products</h3>
                    <div
                        style={{
                            display: "flex",
                            flexDirection: "column",
                ...
                {/* Sales Channels */}
                <div className="card" style={{ padding: 20, borderRadius: "var(--rl)" }}>
                    <h3 style={{ fontSize: 13, fontWeight: 700, color: "var(--ink)", margin: "0 0 14px" }}>Sales by Channel</h3>
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                        {[
                            { name: "WhatsApp", icon: MessageCircle, color: "#25D366", bg: "rgba(37,211,102,0.08)" },
                            { name: "Online Store", icon: TrendingUp, color: "var(--primary)", bg: "var(--primary-lt)" },
                        ].map((ch) => {
                            const Icon = ch.icon;
                            return (
                                <div key={ch.name} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                                            <span style={{ fontSize: 12, fontWeight: 600, color: "var(--ink)" }}>{ch.name}</span>
