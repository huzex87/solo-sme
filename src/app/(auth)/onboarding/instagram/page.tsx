...
export default function InstagramOnboarding() {
    ...
    const handleConfirm = async () => {
        ...
        setLoading(true);
        try {
            // Save imported state to session storage for Signup pick-up
            sessionStorage.setItem('solo_onboarding_import', JSON.stringify(result));

            // If we have a real user session already (rare here), we could finalize now.
            // But usually, unauthenticated users are coming from here.
            router.push(`/signup?import=true&businessName=${encodeURIComponent(result.business_name)}&subdomain=${encodeURIComponent(result.subdomain)}`);
        } catch (err) {
